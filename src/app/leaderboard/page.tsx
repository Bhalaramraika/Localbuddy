...
const RankIndicator = ({ rank }: { rank: number }) => {
  if (rank === 1) return <Crown className="w-6 h-6 text-neon-gold" style={{ filter: 'drop-shadow(0 0 5px var(--neon-gold))' }} />;
...
const ChangeIndicator = ({ change }: { change: number }) => {
  if (change > 0) return <TrendingUp className="w-5 h-5 text-neon-green" />;
  if (change < 0) return <TrendingDown className="w-5 h-5 text-neon-pink" />;
...
const LeaderboardRow = ({ user }: { user: LeaderboardUser }) => {
  ...
  return (
    <div className={cn(
      "flex items-center gap-4 p-3 glass-pill mb-3 transition-all duration-300 hover:bg-white/10 hover:border-neon-cyan border border-transparent rounded-lg cursor-pointer group",
      isTopThree && 'border-neon-gold/30'
    )}>
      <div className="w-10 text-center flex items-center justify-center">
        <RankIndicator rank={user.rank} />
      </div>
      <Image src={avatar?.imageUrl || ''} alt={user.name} width={48} height={48} className="rounded-full border-2 border-neon-cyan/50 group-hover:border-neon-cyan transition-colors" />
      <div className="flex-grow">
        <p className="font-bold text-white text-lg">{user.name}</p>
        <p className="text-sm text-neon-green" style={{textShadow: '0 0 8px var(--neon-green)'}}>{user.score.toLocaleString()} XP</p>
      </div>
      <div className="w-12 text-center flex justify-center items-center gap-1">
        <ChangeIndicator change={user.change} />
        {user.change !== 0 && <span className={cn("text-sm", user.change > 0 ? 'text-neon-green' : 'text-neon-pink')}>{Math.abs(user.change)}</span>}
      </div>
    </div>
  );
};
